...
#define ARG_CHECK_NUMBER(I, VAR) \
    if (args.Length() <= I) { \
        THROW_EXCEPTION("Argument \"" #VAR "\" is mandatory") \
    } else if (!args[I]->IsNumber()) { \
        THROW_EXCEPTION("Argument \"" #VAR "\" must be a valid Number") \
    }
...
#define ARG_CHECK_OBJECT_ATTR_ARRAY(VAR, KEY) \
    v8::Local<v8::String> KEY##_##key = v8::String::New("" #KEY ""); \
    if (!VAR->Has(KEY##_##key)) { \
        THROW_EXCEPTION("Option \"" #KEY "\" is mandatory") \
    } else if (!VAR->Get(KEY##_##key)->IsArray()) { \
        THROW_EXCEPTION("Option \"" #KEY "\" must be a valid Array") \
    }

#define ARG_CHECK_OBJECT_ATTR_NUMBER(VAR, KEY) \
    v8::Local<v8::String> KEY##_##key = v8::String::New("" #KEY ""); \
    if (!VAR->Has(KEY##_##key)) { \
        THROW_EXCEPTION("Option \"" #KEY "\" is mandatory") \
    } else if (!VAR->Get(KEY##_##key)->IsNumber()) { \
        THROW_EXCEPTION("Option \"" #KEY "\" must be a valid Number") \
    }
